...
dotnet ./tools/sonar/SonarScanner.MSBuild.dll begin /o:"andresdigiovanni-github" /k:"andresdigiovanni_ADN.AppSettingsCore" /d:sonar.cs.opencover.reportsPaths="./tests/ADN.AppSettingsCore.Tests/coverage.opencover.xml" /d:sonar.exclusions="**/bin/**/*,**/obj/**/*" /d:sonar.host.url="https://sonarcloud.io" /d:sonar.verbose=true /d:sonar.login=${SONAR_TOKEN}
echo "Running build..."
dotnet build ./src/ADN.AppSettingsCore/
dotnet build ./tests/ADN.AppSettingsCore.Tests/
echo "Running tests..."
dotnet test ./tests/ADN.AppSettingsCore.Tests/ /p:CollectCoverage=true /p:CoverletOutputFormat=opencover
